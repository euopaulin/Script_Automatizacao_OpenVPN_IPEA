...
Este arquivo é uma versão atualizada do `README.md` contendo o script em Python (existente) e a inclusão do script em Bash.
...
## Script em Bash
Tamnbém fiz uma versão em `Script bash - Linux/script.bash` presente neste repositório.

Esse em bash segue a mesma lógica do em Python:

- Atualiza o sistema: executa `apt update` e `apt upgrade -y` para trazer pacotes e correções.
- Instala plugin OpenVPN para o NetworkManager (`network-manager-openvpn`) e reinicia o NetworkManager para aplicar o plugin.
- Instala o cliente de área de trabalho Remmina e plugins úteis (`remmina-plugin-rdp`, `remmina-plugin-secret`) para conexões RDP e gerenciamento de credenciais.

**Recomendações de uso:**

- Execute com privilégios administrativos: `sudo bash "Script em bash - Linux/script.bash"`.
- Teste em um ambiente controlado antes de usar em produção.
- Ajuste nomes de pacotes se estiver usando outra distribuição ou versões diferentes do Ubuntu/Debian.

Para ver o conteúdo completo do script, abra `Script em bash - Linux/script.bash` no repositório.
